...
describe("Wrapper Contract", function () {
  ...
  it("Should revert to wrap erc721 token twice and get info", async function () {
    const id = await getId(erc721);
    await assetContract.grantRole(
      AssetManagerAccess,
  ...
  it("Should revert to wrap erc1155 if not owner", async function () {
    await expect(wrapperContract.wrapERC1155(Sand1155, SandTokenId, 10000)).to
      .be.reverted;
  });

  it("Should revert to wrap erc1155 with zero balance", async function () {
    await expect(wrapperContract.wrapERC1155(Sand1155, SandTokenId, 0)).to.be
      .reverted;
  });
  ...
  it("Should revert to wrap erc1155 contract address is not IERC1155", async function () {
    await expect(wrapperContract.wrapERC1155(Ens721, SandTokenId, 10000)).to.be
      .reverted;
  });

  it("Should wrap erc1155 token and get info", async function () {
    const id = await getId(erc1155);
    ...
    await wrapperContract
      .connect(signer1155)
      .wrapERC1155(Sand1155, SandTokenId, balance);

    expect(
      await assetContract.subBalanceOf(signer1155.getAddress(), id, SandTokenId)
    ).to.be.eq(balance);
    ...
    expect(info[0]).to.be.eq(Sand1155);
    expect(info[1][0]).to.be.eq(SandTokenId);
    expect(info[2][0]).to.be.eq(balance);
    expect(info[3][0]).to.be.eq(balance);

    await resetFork();
  });

  it("Should wrap erc1155 token twice and get info", async function () {
    await erc1155
      .connect(signer1155)
      .setApprovalForAll(await wrapperContract.getAddress(), true);
    ...
    await wrapperContract
      .connect(signer1155)
      .wrapERC1155(Sand1155, SandTokenId, 1);
  ...
  it("Should revert to unwrap erc1155 token when have no enough balance", async function () {
    const id = await getId(erc1155);
    ...
    await expect(
      wrapperContract.connect(signer1155).unwrapERC1155(id, SandTokenId, 1)
    ).to.be.reverted;
  ...
  it("Should unwrap erc1155 token", async function () {
    ...
    await wrapperContract.connect(signer1155).unwrapERC1155(id, SandTokenId, 1);

    expect(await assetContract.totalSubSupply(id, SandTokenId)).to.be.eq(0);
  ...
  it("Should revert batch wrap erc1155 token without array parity", async function () {
    ...
    await expect(
      wrapperContract
        .connect(signer1155)
        .batchWrapERC1155([Sand1155], [SandTokenId, SandTokenId], [10000])
    ).to.be.reverted;

    await expect(
      wrapperContract
        .connect(signer1155)
        .batchWrapERC1155([Sand1155, Sand1155], [SandTokenId], [10000])
    ).to.be.reverted;
  });
  ...
  it("Should revert unwrap20 with invalid id", async function () {
    const id = getId(erc20);
    await expect(wrapperContract.connect(signer20).unwrapERC20(id, 10000)).to.be
      .reverted;
  });

  it("Should revert unwrap721 with invalid id", async function () {
    const id = getId(erc721);
    await expect(wrapperContract.connect(signer721).unwrapERC721(id, 1)).to.be
      .reverted;
  });

  it("Should revert unwrap1155 with invalid id", async function () {
    const id = getId(erc1155);
    await expect(wrapperContract.connect(signer1155).unwrapERC1155(id, 1, 1)).to
      .be.reverted;
  });
});
