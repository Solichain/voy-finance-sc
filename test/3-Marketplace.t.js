...
describe("Marketplace", function () {
  ...
  it("Should revert on listing with invalid token address", async function () {
    await expect(
      marketplaceContract.list(
        1,
        1,
        await createList(10, 1, 100, ethers.ZeroAddress)
      )
    ).to.reverted;
  });
  ...
  it("Should revert on listing with invalid sale price", async function () {
    await expect(
      marketplaceContract.list(
        1,
        1,
        await createList(0, 1, 100, await stableTokenContract.getAddress())
      )
    ).to.reverted;
  });
    ...
    await expect(
      marketplaceContract
        ...
        .list(
          1,
          1,
          await createList(100, 100, 100, stableTokenContract.getAddress())
        )
    ).to.be.reverted;
  });
    ...
    await expect(
      marketplaceContract
        ...
        .list(
          1,
          1,
          await createList(100, 100, 0, stableTokenContract.getAddress())
        )
    ).to.be.reverted;
  });
    ...
    await expect(
      marketplaceContract
        ...
        .list(
          1,
          1,
          await createList(100, 100, 200, stableTokenContract.getAddress())
        )
    ).to.be.reverted;
  });
    ...
    await expect(
      marketplaceContract
        ...
        .list(
          1,
          1,
          await createList(100, 200, 100, stableTokenContract.getAddress())
        )
    ).to.be.reverted;
  });
  ...
  it("Should revert on listing and buying with less than Min. fraction", async function () {
    ...
    await marketplaceContract
      ...
      .list(
        1,
        1,
        await createList(100, 10, 10, stableTokenContract.getAddress())
      );
  ...
  it("Should revert to batch List without array parity", async function () {
    ...
    const list = await createList(100, 5, 5, stableTokenContract.getAddress());

    await expect(marketplaceContract.batchList([1], [1, 1], [list, list])).to
      .reverted;

    await expect(
      marketplaceContract.connect(user1).batchList([1, 1], [1], [list, list])
    ).to.reverted;

    await expect(marketplaceContract.batchList([1, 1], [1, 1], [list])).to
      .reverted;
  });
  ...
  it("Should revert to batch List more than 30 limit", async function () {
    ...
    const list = await createList(100, 1, 1, stableTokenContract.getAddress());

    await expect(
      marketplaceContract
        .connect(user1)
        .batchList(Array(31).fill(1), Array(31).fill(1), Array(31).fill(list))
    ).to.reverted;
  });
  ...
  it("Should revert to batch Buy and Unlist more than 30 limit", async function () {
    ...
    await expect(
      marketplaceContract.batchUnlist(Array(31).fill(1), Array(31).fill(1))
    ).to.reverted;
  ...
  it("Should batch create properties and batch list and unlisted", async function () {
    ...
    await marketplaceContract.connect(user1).batchUnlist([1, 1, 1], [1, 2, 3]);
  });
  ...
  it("Should return the listed info struct", async function () {
    ...
    await marketplaceContract
      ...
      .list(
        1,
        1,
        await createList(100, 1, 1, stableTokenContract.getAddress())
      );
  ...
  it("Should create asset, list it and selling it to buyer through Marketplace", async function () {
    ...
    const list = await createList(2000, 1, 1, stableTokenContract.getAddress());
    ...
    await expect(
      await marketplaceContract.connect(buyer).buy(1, 1, 1, user1.getAddress())
    ).not.to.be.reverted;
    ...
    expect(await assetContract.subBalanceOf(buyer.getAddress(), 1, 1)).to.eq(1);
  });
